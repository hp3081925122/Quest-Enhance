...
public final class HiddenDependencyLineMenus {
    ...
    public static List<ContextMenuItem> append(
        ...
        source_quest.streamDependencies().forEach(dependency_object -> {
            Quest dependency = linkedQuest(dependency_object);
            if (dependency == null || dependency == source_quest) {
                return;
            }
            chapter_nodes.stream()
        ...
        for (Movable dependent : chapter_nodes) {
            ...
            if (dependent == source || dependent_quest == null || !dependent_quest.streamDependencies()
                    .anyMatch(dependency_object -> linkedQuest(dependency_object) == source_quest)) {
                continue;
            }
            addLineItem(
                    line_items,
    ...
    private static void open(
        ...
        BooleanConfig[] hidden_config = new BooleanConfig[1];
        hidden_config[0] = group.addBool(
                "hidden",
                hidden[0],
                value -> {
                    hidden[0] = value || reveal_on_hover[0];
                    hidden_config[0].setCurrentValue(hidden[0]);
                },
                false
        );
        ...
        group.addBool("reveal_on_hover", reveal_on_hover[0], value -> {
                    ...
                    if (value) {
                        hidden[0] = true;
                        hidden_config[0].setCurrentValue(true);
                    }
                }, false)

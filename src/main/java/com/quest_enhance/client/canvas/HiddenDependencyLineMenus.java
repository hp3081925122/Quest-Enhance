...
public final class HiddenDependencyLineMenus {
    ...
    public static List<ContextMenuItem> append(
        ...
        source.streamDependencies()
                ...
                .filter(dependency -> dependency != null && dependency != source)
                .distinct()
                .forEach(dependency -> addLineItem(
                        line_items,
                        line_keys,
                        quest_screen,
                        chapter,
        ...
        for (Quest dependent : chapter.getQuests()) {
            if (dependent == source || !dependent.streamDependencies()
                    .map(QuestObject::getRelatedQuest)
                    .anyMatch(dependency -> dependency == source)) {
                continue;
            }
            addLineItem(
                    line_items,
